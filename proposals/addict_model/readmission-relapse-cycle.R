...
addictionSystem <- function(el, nl){ 
  pmap_df(el, function(from, to, fromStabilized, toType, rate){
    randomNum <- runif(1)
...
nl <- bind_rows(people, rehab, outRehab)
el <- tibble(
  from = people$id,
  to = 'nothing',
  fromStabilized = F,
