...
g_rtwt <- igraph::graph_from_data_frame(retweet_network_wt, T) %>%
  ...
  {temp <- igraph::as_data_frame(., 'both'); igraph::graph_from_data_frame(temp$edges, T, dplyr::arrange(temp$vertices, pr))} %>%
  {
    set.seed(4321); 
...
tweet_hashtag_g_filtered %>%
  ...
  plot( 
     ...
     sub = "Connections where Trolls used a hashtag less than 6 times were removed"
     )
```

## Project Bipartite to get 
...
## Take a sample of each component

```{r, echo = F}
hashtag_g_components <- igraph::components(hashtag_g)

hashtag_g_components %>% 
  igraph::groups() %>%
  purrr::map(function(x){
    tmp <- V(hashtag_g)[name %in% x];
    tmpd <- tmp$degree
    names(tmpd) <- tmp$name;
    tmpd %>%
      sort(T) %>%
  ...
  tibble::enframe('Component', 'Hashtag') %>%
  tidyr::unnest() 
```
